...
#ifndef HASHCOMP_RANDOM_H
#define HASHCOMP_RANDOM_H
...
public:
    ...
    Random(double eps)
            : last_n(0), EPS(eps / std::sqrt(2.0)),
              buffer_size_limit(std::ceil(1.0 / EPS * std::sqrt(std::log(1.0 / DELTA) / std::log(2.0)))),
              layer_count_limit(std::ceil(
                      std::log(1.0 / EPS * std::sqrt(std::log(1 / DELTA) / std::log(2.0)) * 5.0 / 12.0) /
                      std::log(2.0))), buffer_count_limit(layer_count_limit + 1), next_new_sample(0), next_sampled(0),
              prob_mask(0), prob_mask_width(0), prob_bits_length(0), coin_bits_length(0) {
    ...
    struct HeapEntry {
        ...
        HeapEntry(const typename buffer_t::iterator &_iter, const typename buffer_t::iterator &_iterend,
                  long long _weight)
